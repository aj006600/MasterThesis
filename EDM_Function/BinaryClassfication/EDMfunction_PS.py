...
"""
'PS' means using 'cv=ps' in GridSearchCV(), it can be only used in BALANCED DATA, like Tp=1.
...
def OverSampling(Library, target):
    """
    ...
    resampled_df.insert(0, 'Date', range(1, len(resampled_df) + 1)) # asign 'Date': start from 1

    return resampled_df

def UnderSampling(Library, target):
    """
