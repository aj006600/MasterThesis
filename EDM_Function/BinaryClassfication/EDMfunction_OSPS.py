...
"""
'OSPS' means using OverSampling() and using 'cv=ps' in GridSearchCV(). 
It can be used in IMBALANCED DATA, like Tp=5, Tp=10.

The functions that are adjusted include the following: EDM_ModelSelection(), EDM_TrainningModel()
"""

########################################################################################

def OverSampling(train_df, target):
    """
    Note: Samples extracted with duplicates will be appended at the end.
    """
...
def UnderSampling(train_df, target):
    """
    Note: The time series order has been completely disrupted and cannot be used for viewing. 
          It can only be used for logistic regression.
    """
...
def ConcateLibPred(Library, Prediction, th): 
    ...
    if th <= 0:
        ...
        Lib_Pred_df['Date'] = pd.to_datetime(Lib_Pred_df['Date']) # ver.Date
...
def EDM_TargetOED(data, target, valid_interval):
    """
    這邊要在思考一下EmbedDimension的lib和pred
    """
...
def EDM_FeatureSelection(data, target, train_feature, TargetOED, E_max):
    ...
    f_selec.index = pd.Index([f'{target}']) 
    
    return f_selec

########################################################################################

def EDM_CreateNewDF(data, target, f_selec, max_lag, th):
    ...
    ### Create ML_df using the data from Embed_df at time (t-0). ###
    if th <= 0 : # to search for the view in EDM
        ML_df = Embed_df.copy()
        ML_df['Date'] = pd.to_datetime(ML_df['Date']) # Set the index as 'Date'
        ML_df.set_index('Date', inplace=True)
        ML_df = ML_df.filter(like="(t-0)") # keep only the columns for (t-0)
    
    if th > 0 : # no need to search for a view
        ML_df = Embed_df.copy()
        ML_df = ML_df.dropna().reindex()
        ML_df = ML_df.filter(like="(t-0)") # keep only the columns for (t-0)
...
def EDM_ModelSelection(ML_df, target, dmatrix, theta_seq, Tp):
    ...
    for theta in theta_seq:
        ...
        train_df = ML_df_new.iloc[:-60]
        valid_df = ML_df_new.iloc[-60:]
        resampled_ML_df = OverSampling(train_df=train_df, target='ans(t-0)')

        resampled_ML_df = pd.concat([resampled_ML_df, valid_df], axis=0, ignore_index=True)
        resampled_ML_df = resampled_ML_df.reset_index(drop=True)

        X = resampled_ML_df.iloc[:, :-1]
        y = resampled_ML_df.iloc[:, -1]
        val_fold = [-1] * (len(X)-60) + [0] * 60 # the last 60 records are used as the validation set # ver.ps
        ps = PredefinedSplit(test_fold=val_fold) # ver.ps
...
def EDM_TrainningModel(ML_df, target, dmatrix, theta, param, Tp):
    ...
    # ML_df_new = ML_df_new.drop(columns=[f'{target}(t-0)'])

    # Split into train and validation sets. #
    resampled_ML_df = OverSampling(train_df=ML_df_new, target='ans(t-0)')
